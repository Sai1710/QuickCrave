...
const OrderCard = ({ order, getOrders }) => {
  ...
  const acceptOrder = () => {
    axios
      .put(`api/v1/vendor/orders/${order.id}/accept_order`)
      .then((res) => {
        console.log(res);
        getOrders();
      })
  ...
  const rejectOrder = () => {
    axios
      .put(`api/v1/vendor/orders/${order.id}/reject_order`)
      .then((res) => {
        console.log(res);
        getOrders();
      })
  ...
  const readyforDelivery = () => {
    axios
      .put(`api/v1/vendor/orders/${order.id}/ready_for_delivery_order`)
      .then((res) => {
        console.log(res);
        getOrders();
      })
  ...
  const startPreparing = () => {
    axios
      .put(`api/v1/vendor/orders/${order.id}/preparing_order`)
      .then((res) => {
        console.log(res);
        getOrders();
      })
  ...
  const renderButtons = (status) => {
    switch (status) {
      case "pending":
        return (
          <View className="flex-row align-middle justify-center">
            <TouchableOpacity
              className="flex-1 p-2 bg-red-700 rounded-md mt-2 mr-1"
              onPress={rejectOrder}
            >
              <Text className="text-white  text-center font-bold">Reject</Text>
            </TouchableOpacity>
            <TouchableOpacity
              className=" flex-1 p-2 bg-green-700 rounded-md mt-2 ml-1"
              onPress={acceptOrder}
            >
      ...
      case "approved":
        return (
          <TouchableOpacity
            className="p-2 bg-green-700 rounded-md mt-2 mr-1"
  ...
  return (
    <View className="p-4 mb-4 border rounded-lg  bg-white border-green-700 border-opacity-50">
      {order.payment_status === "paid" ? (
        <Text className="absolute top-2 right-2 bg-green-700 py-1 px-2 rounded-md text-white font-bold">
          Paid
        </Text>
      ) : (
        <Text className="absolute top-2 right-2 bg-red-700 py-1 px-2 rounded-md text-white font-bold">
          Unpaid
        </Text>
      )}
      ...
      <Text>Ordered by: {order.customer_name}</Text>
      <Text
        className={`text-base ${
      ...
      <TouchableOpacity
        className="self-end p-2 border border-green-700 rounded-md mt-2"
